...
def upgrade() -> None:
    ...
    # Check if username column exists (Identity architecture compatibility)
    ...
    if has_username:
        ...
        # Identity architecture: use identity_id -> identities.username
        conn.execute(
            sa.text("""
            INSERT INTO participants (id, type, ref_id, display_name, avatar_url)
            SELECT gen_random_uuid(), 'user', u.id, COALESCE(u.display_name, i.username), u.avatar_url
            FROM users u
    ...
    conn.execute(
        sa.text("ALTER TABLE chat_sessions ADD COLUMN IF NOT EXISTS participant_id UUID REFERENCES participants(id)")
    )
    conn.execute(sa.text("ALTER TABLE chat_sessions ADD COLUMN IF NOT EXISTS peer_agent_id UUID REFERENCES agents(id)"))
    conn.execute(sa.text("ALTER TABLE chat_sessions ADD COLUMN IF NOT EXISTS external_conv_id VARCHAR(200)"))

    # 5. Add participant_id to chat_messages
    conn.execute(
        sa.text("ALTER TABLE chat_messages ADD COLUMN IF NOT EXISTS participant_id UUID REFERENCES participants(id)")
    )
    ...
    if has_messages:
        ...
        for agent_a, agent_b in pairs:
            ...
            part_a = conn.execute(
                sa.text("SELECT id FROM participants WHERE type = 'agent' AND ref_id = :ref"), {"ref": str(agent_a)}
            ).scalar()
            ...
            creator_id = conn.execute(
                sa.text("SELECT creator_id FROM agents WHERE id = :id"), {"id": str(agent_a)}
            ).scalar()
            ...
            for sender_id, content, created_at in msgs:
                sender_part = conn.execute(
                    sa.text("SELECT id FROM participants WHERE type = 'agent' AND ref_id = :ref"),
                    {"ref": str(sender_id)},
                ).scalar()
