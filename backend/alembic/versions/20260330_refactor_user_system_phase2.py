...
def upgrade() -> None:
    ...
    # Check if required columns exist (Identity architecture compatibility)
    user_columns = [c["name"] for c in inspector.get_columns("users")]

    # Only run migration if legacy columns exist
    users_data = []
    if "email" in user_columns or "primary_mobile" in user_columns:
        email_col = "email" if "email" in user_columns else "NULL::VARCHAR"
        phone_col = "primary_mobile" if "primary_mobile" in user_columns else "NULL::VARCHAR"
        username_col = "username" if "username" in user_columns else "NULL::VARCHAR"
        password_col = "password_hash" if "password_hash" in user_columns else "NULL::VARCHAR"
        email_verified_col = "email_verified" if "email_verified" in user_columns else "FALSE"

        result = conn.execute(
            sa.text(f"""
            SELECT id, {email_col}, {phone_col}, {username_col}, {password_col}, {email_verified_col}, is_active, role 
            FROM users 
