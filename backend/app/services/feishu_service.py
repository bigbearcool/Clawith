...
class FeishuService:
    ...
    async def login_or_register(
        self, db: AsyncSession, feishu_user: dict, tenant_id: str | None = None
    ) -> tuple[User, str]:
        ...
        # 3. Fallback: find by email via Identity
        if not user and fs_email:
            identity_result = await db.execute(select(Identity).where(Identity.email == fs_email))
        ...
        if user:
            ...
            existing_r = await db.execute(select(Identity).where(Identity.username == username))
