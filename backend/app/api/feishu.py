...
async def process_feishu_event(agent_id: uuid.UUID, body: dict, db: AsyncSession):
    ...
    if event_type == "im.message.receive_v1":
        ...
        if msg_type == "text":
            ...
            try:
                cardkit_card_id = await feishu_service.create_card_entity(config.app_id, config.app_secret, init_card)
                cardkit_sequence = 1
...
async def _handle_feishu_file(db, agent_id, config, message, sender_open_id, chat_type, chat_id):
    ...
    if msg_type == "audio":
        ...
        async with _async_session() as db:
            ...
            # Get Tencent Voice credentials from tenant settings
            from app.models.tenant_settings import TenantSetting

            tencent_settings_r = await db.execute(
                _select(TenantSetting).where(
                    TenantSetting.tenant_id == agent_obj.tenant_id, TenantSetting.key == "tencent_voice_key"
                )
            )
            tencent_settings = tencent_settings_r.scalar_one_or_none()

            if not tencent_settings or not tencent_settings.value:
                logger.error(f"[Feishu Audio] Tencent Voice key not configured")
                _reply_to = chat_id if chat_type == "group" else sender_open_id
                _rid_type = "chat_id" if chat_type == "group" else "open_id"
                await feishu_service.send_message(
                    config.app_id,
        ...
        try:
            ...
            # Convert wav to opus (Feishu requires opus for audio messages)
