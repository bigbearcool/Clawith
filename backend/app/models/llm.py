...
class LLMModel(Base):
    ...
    streaming_tool_calls_unreliable: Mapped[bool] = mapped_column(Boolean, default=False)
    temperature: Mapped[float | None] = mapped_column(Float, nullable=True)
    request_timeout: Mapped[int | None] = mapped_column(Integer, nullable=True)
    max_output_tokens: Mapped[int | None] = mapped_column(Integer, nullable=True)
    # Tencent Cloud Voice settings
    tencent_secret_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
    tencent_secret_key: Mapped[str | None] = mapped_column(String(100), nullable=True)
    ...
    enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    supports_vision: Mapped[bool] = mapped_column(Boolean, default=False)
    streaming_tool_calls_unreliable: Mapped[bool] = mapped_column(
        Boolean, default=False
    )  # Models like MiniMax that don't support native function calling
    temperature: Mapped[float | None] = mapped_column(Float, nullable=True)
