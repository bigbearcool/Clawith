# xiaosheng-docx Environment Setup & Initialization Script (Windows PowerShell)
...
Write-Host "============================================"
Write-Host "  xiaosheng-docx Setup & Initialization (Windows)"
...
# --- Build Project ---
Step "Building xiaosheng-docx .NET project"
...
if (-not $SkipVerify) {
    ...
    $testOutput = Join-Path $env:TEMP "xiaosheng-docx-setup-test-$PID.docx"

    Info "Creating a test document..."
    Push-Location $DotnetDir
    $testResult = & dotnet run --project XiaoShengAIDocx.Cli -- create --type report --output $testOutput --title "Setup Test" 2>&1
    ...
    if ($testExitCode -eq 0 -and (Test-Path $testOutput)) {
        ...
        Log "Test passed - xiaosheng-docx is ready to use!"
    } else {
...
Write-Host ""
Write-Host "  Usage:"
Write-Host "    dotnet run --project $DotnetDir\XiaoShengAIDocx.Cli -- create --type report --output my_report.docx"
Write-Host ""
