#!/usr/bin/env bash
# xiaosheng-docx Environment Setup & Initialization Script
...
verify_installation() {
    ...
    local test_output="/tmp/xiaosheng-docx-setup-test-$$.docx"

    info "Creating a test document..."
    if cd "$DOTNET_DIR" && dotnet run --project XiaoShengAIDocx.Cli -- create \
...
print_summary() {
    ...
    echo ""
    echo "  Usage:"
    echo "    dotnet run --project $DOTNET_DIR/XiaoShengAIDocx.Cli -- create --type report --output my_report.docx"
...
main() {
    echo "============================================"
    echo "  xiaosheng-docx Setup & Initialization"
