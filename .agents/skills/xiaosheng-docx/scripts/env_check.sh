#!/usr/bin/env bash
# xiaosheng-docx Quick Environment Check
# Cross-platform: macOS, Linux, WSL, Git Bash
# Run this BEFORE any xiaosheng-docx operation. Use setup.sh for initial installation.
set -euo pipefail
...
echo "=== xiaosheng-docx Environment Check ==="
echo ""
...
if [ -d "$DOTNET_DIR" ]; then
    if [ -f "$DOTNET_DIR/XiaoShengAIDocx.Cli/bin/Debug/net10.0/XiaoShengAIDocx.Cli.dll" ] || \
       [ -f "$DOTNET_DIR/XiaoShengAIDocx.Cli/bin/Debug/net8.0/XiaoShengAIDocx.Cli.dll" ]; then
        printf "[OK]      %-14s built\n" "project"
    else
        # Try restore + build
